...
public class FireTeraShard extends SimplePolymerItem {
    PolymerModelData modelData;
    public FireTeraShard(Settings settings, Item polymerItem) {
    ...
    public int getPolymerCustomModelData(ItemStack itemStack, @Nullable ServerPlayerEntity player){
        this.modelData = TeraObjects.fireTeraShardModelData;
    ...
    public ActionResult useOnEntity(ItemStack itemStack, PlayerEntity playerEntity, LivingEntity livingEntity, Hand hand) {
        if(livingEntity instanceof PokemonEntity pokemonEntity){
            if(!pokemonEntity.getExposedSpecies().getName().equalsIgnoreCase("ogerpon") && !pokemonEntity.getExposedSpecies().getName().equalsIgnoreCase("terapagos")) {
                pokemonEntity.getPokemon().setTeraType(TeraTypes.getFIRE());
                ItemDeleter.deleteItemStack(playerEntity);
                playerEntity.sendMessage(Text.literal("Pokemon´s Teratype Changed to " + TeraTypes.getFIRE().getDisplayName()).formatted(Formatting.GREEN), true);
            }else{
