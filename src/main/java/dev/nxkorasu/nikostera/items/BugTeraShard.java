...
public class BugTeraShard extends SimplePolymerItem {
    PolymerModelData modelData;
    public BugTeraShard(Settings settings, Item polymerItem) {
    ...
    public int getPolymerCustomModelData(ItemStack itemStack, @Nullable ServerPlayerEntity player){
        this.modelData = TeraObjects.bugTeraShardModelData;
    ...
    public ActionResult useOnEntity(ItemStack itemStack, PlayerEntity playerEntity, LivingEntity livingEntity, Hand hand) {
        if(livingEntity instanceof PokemonEntity pokemonEntity){
            pokemonEntity.getPokemon().setTeraType(TeraTypes.getBUG());
            ItemDeleter.deleteItemStack(playerEntity);
            playerEntity.sendMessage(Text.literal("Pokemon´s Teratype Changed to "+TeraTypes.getBUG().getDisplayName()).formatted(Formatting.GREEN),true);
        }
