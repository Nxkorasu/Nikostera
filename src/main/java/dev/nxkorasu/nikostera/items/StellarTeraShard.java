...
public class StellarTeraShard extends SimplePolymerItem {
    PolymerModelData modelData;
    public StellarTeraShard(Settings settings, Item polymerItem) {
    ...
    public int getPolymerCustomModelData(ItemStack itemStack, @Nullable ServerPlayerEntity player){
        this.modelData = TeraObjects.stellarTeraShardModelData;
    ...
    public ActionResult useOnEntity(ItemStack itemStack, PlayerEntity playerEntity, LivingEntity livingEntity, Hand hand) {
        if(livingEntity instanceof PokemonEntity pokemonEntity){
            if(!pokemonEntity.getExposedSpecies().getName().equalsIgnoreCase("ogerpon")) {
                ...
                playerEntity.sendMessage(Text.literal("Pokemon´s Teratype Changed to Stellar").formatted(Formatting.GREEN), true);
            }else{
